...
public class PerspectiveCamera extends Camera {
    ...
    public void initView() {
        ...
        basisW.sub(viewDir);
        basisU.set(viewUp).cross(basisW);
        basisV.set(basisW).cross(basisU);
    ...
    public void getRay(Ray outRay, double inU, double inV) {
        ...
        //    as well as the projection distance.
    	 ...
    	 double uDis = (inU - 0.5) * viewWidth;
    	 double vDis = (inV - 0.5) * viewHeight;
    	 
    	 Vector3d rayDirection = new Vector3d();
    	 //ray.direction = ¨Cd w + u u + v v
    	 rayDirection.set(centerDir).mul(projDistance);
    	 rayDirection.addMultiple(uDis, basisU);
    	 rayDirection.addMultiple(vDis, basisV);
    	 
    	 rayDirection.normalize();
    	 
    	 outRay.direction.set(rayDirection);
